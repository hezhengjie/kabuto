    ...
    return new Promise((resolve, reject) => {
      ...
      req.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
          ...
          const sourceUrls = html
            .match(resourceReg)
            .filter(resource => {
              return (
                resource.indexOf('rel="dns-prefetch"') < 0 &&
                resource.indexOf("rel='dns-prefetch'") < 0
              );
            })
            .map(link => {
...
function parseHtml(html: string): any[] {
  ...
  return Array.from(dom.querySelectorAll("script,link,img"))
    .filter(source => {
...
export { getSourceUrls, getSourceUrlsByWorker, parseHtml };
