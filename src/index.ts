...
class Kabuto {
  ...
  ignores: any[] = [];
  noquery: boolean = true; // 参数不同的链接认为是同一个页面，不进行资源的预加载，只进行页面预加载
  timeout: number = 2000;
  constructor(
    config: {
      timeout?: number | void;
      urls?: string[] | void;
      origins?: string[] | boolean | void;
      el?: HTMLElement | void;
      ignores?: any[] | void;
      observeTime?: number | void;
      noquery?: boolean | void;
    } | void
  ) {
    const {
      urls = this.urls,
  ...
  add(url: string) {
    ...
    if (checkUrlType(url) === "page") {
      ...
      if (this.noquery) {
        // 判断是否有同路径页面
        const sameUrl = this.pageUrl.find(pageUrl => {
          return checkSameUrl(pageUrl, url);
        });
        if (sameUrl) {
          return;
        }
      ...
      if (checkSameOrigin(url)) {
        ...
        if (window.Worker) {
          ...
          console.warn(
            "注意：当前环境不支持web worker,页面的解析将在主线程空闲时进行"
          );
